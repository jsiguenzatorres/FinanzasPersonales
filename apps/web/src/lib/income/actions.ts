...
export async function editIncomeAction(formData: FormData) {
  ...
  if (!parsed.success) {
    const message = parsed.error.issues[0]?.message ?? 'Datos inválidos.';
    redirect(`/app/ingresos/${incomeId}/editar?error=` + encodeURIComponent(message));
  }
  ...
  if (incomeError) {
    redirect(`/app/ingresos/${incomeId}/editar?error=` + encodeURIComponent(incomeError.message));
  }

  // Mantener sincronizada la transacción de cobro vinculada (si existe).
  if (parsed.data.is_collected && existing?.transaction_id) {
    // Seguía cobrado: actualizar la transacción existente.
    await supabase
      .from('transactions')
      .update({
        account_id: parsed.data.account_id,
  ...
  } else if (parsed.data.is_collected && !existing?.transaction_id) {
    // Antes no estaba cobrado, ahora sí: crear la transacción de cobro.
    ...
    if (tx) {
      await supabase.from('income_entries').update({ transaction_id: tx.id }).eq('id', incomeId);
    }
  } else if (!parsed.data.is_collected && existing?.transaction_id) {
    // Ya no está cobrado: eliminar (soft-delete) la transacción de cobro.
    await supabase
      .from('transactions')
      .update({ deleted_at: new Date().toISOString() })
      .eq('id', existing.transaction_id);
    await supabase.from('income_entries').update({ transaction_id: null }).eq('id', incomeId);
  }

  revalidatePath('/app/ingresos');
  revalidatePath('/app/cuentas');
  revalidatePath('/app');
  redirect(`/app/ingresos/${incomeId}`);
}
